...
export const Input: React.FC<Props> = (props) => {
  const { label, placeholder, value, onChange, error, disableError } = props
  const renderInput = () => {
    switch (props.type) {
      case undefined:
      case 'text':
        return <input j-input-text placeholder={placeholder} value={value} onChange={e => onChange?.(e.target.value)} />
      case 'emoji':
      ...
      case 'sms_code':
        return (
          <div flex gap-x-16px>
            ...
            <button type="button" max-w="[calc(60%-8px)]" shrink-0 j-btn
